    ...
    <section id="services" className="py-16 bg-black relative overflow-hidden">
      ...
      <button
        ...
        className="absolute left-3 sm:left-4 top-1/2 transform -translate-y-1/2
                   bg-pink-500 text-white p-3 sm:p-4 rounded-full z-20
                   hover:bg-pink-400 transition text-xl sm:text-2xl shadow-lg border-2 border-white/10"
      >
        ‹
      </button>
      <button
      ...
      <div
        ref={scrollRef}
        className="flex space-x-4 sm:space-x-6 lg:space-x-8 overflow-x-auto px-4 sm:px-6 lg:px-10 py-6 sm:py-8 scrollbar-hide scroll-smooth touch-pan-x"
      >
        {servicesData.map(({ key, emoji, titleKey, descriptionKey }, i) => (
          <div
            key={key}
            className="group min-w-[240px] sm:min-w-[300px] lg:min-w-[360px] flex-shrink-0 \
                       bg-[#18181b] rounded-2xl shadow-lg hover:shadow-2xl transition-shadow duration-200 \
                       p-6 sm:p-7 lg:p-8 flex flex-col relative overflow-hidden border-2 border-transparent hover:border-pink-500 h-full"
            data-aos="fade-up"
          ...
          >
            ...
            <div className="flex justify-center mb-3 sm:mb-4">
              <div className="w-16 h-16 sm:w-20 sm:h-20 rounded-full bg-pink-100 flex items-center justify-center text-4xl sm:text-5xl transition-transform duration-300 group-hover:scale-110 group-hover:rotate-6 shadow-md">
                <span role="img" aria-label={t(titleKey)}>{emoji}</span>
              </div>
            </div>
            <h3 className="text-xl sm:text-2xl font-bold text-pink-500 mb-2 text-center relative z-10">
              {t(titleKey)}
            </h3>
            <p className="text-gray-200 flex-1 mb-5 sm:mb-6 text-center relative z-10 text-sm sm:text-base">
              {t(descriptionKey)}
            </p>
            <a
              href="#contact"
              className="mt-auto inline-block w-full text-center py-3 bg-pink-500 text-white rounded-lg font-semibold hover:bg-pink-400 transition relative z-10 text-base sm:text-lg shadow-md border-2 border-pink-500/0 hover:border-pink-500"
            >
              {t('book')}
            </a>
          </div>
